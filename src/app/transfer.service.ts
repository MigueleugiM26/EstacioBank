  ...
  async transferMoney(recipient: string, amount: string): Promise<void> {
    ...
    try {
      ...
      if (currentUserDocSnap.exists()) {
        ...
        let recipientDocs = await getDocs(
          query(
            this.usersCollectionRef,
            where(recipientQueryField, '==', recipient)
          )
        );

        if (recipientDocs.empty) {
          recipientQueryField = 'chaveAleatoria';
          recipientDocs = await getDocs(
            query(
              this.usersCollectionRef,
              where(recipientQueryField, '==', recipient)
            )
        ...
        if (!recipientDocs.empty) {
          ...
          const isConfirmed = await this.presentConfirmationAlert(
            quantia,
            destinatario,
            cpf,
            chaveAleatoria,
          ...
          if (isConfirmed) {
            ...
            let recipientDinheiro =
              Number(recipientDocs.docs[0].data()['dinheiro']) || 0;
    ...
    try {
      ...
      if (currentUserDocSnap.exists()) {
        ...
        if (currentUserDinheiro >= 50) {
          alert(
            'Só é possível receber um empréstimo caso você tenha menos que R$50,00.'
          );
          return;
        }
